...
"""Fold a PaliGemma mmproj GGUF into a pi0 checkpoint GGUF, producing one
self-contained file whose SigLIP tower is built in-tree by src/models/pi0.cpp
(no llama.cpp clip.cpp). Use this when you only have the released pi0 + mmproj
GGUFs; convert_pi0_to_gguf.py bakes the same tensors straight from safetensors.

The clip tensor names (v.blk.*, mm.input_projection.*) are remapped to the
in-tree vit.* / mm.proj.* scheme, and the SigLIP geometry is copied into pi0.* KV.
"""
import argparse
import sys
...
# clip.cpp mmproj tensor name -> in-tree pi0 name
FIXED = {
...
def main():
    ...
    ap.add_argument("ckpt", help="pi0-*.gguf produced by convert_pi0_to_gguf.py")
    ...
    if not ck.get_field("pi0.architecture"):
        sys.exit(f"{args.ckpt} is not a pi0 GGUF (pi0.architecture missing)")

    w = gguf.GGUFWriter(args.out, arch="pi0")
    ...
    # SigLIP geometry from the mmproj -> pi0.vit_* KV
    ...
    ntok = (img // patch) ** 2
    w.add_uint32("pi0.vit_hidden", int(vh))
    w.add_uint32("pi0.vit_layers", int(vl))
    w.add_uint32("pi0.vit_heads", int(vhd))
    w.add_uint32("pi0.image_size", int(img))
    w.add_uint32("pi0.patch_size", int(patch))
    w.add_uint32("pi0.n_img_tokens", int(ntok))
    w.add_float32("pi0.vit_ln_eps", float(eps))
