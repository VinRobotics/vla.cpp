...
VIT_LN_EPS = 1e-6
LM = dict(hidden=896, layers=24, q_heads=14, kv_heads=2, head_dim=64, inter=4864,
          rope_theta=1_000_000.0, rms_eps=1e-6, vocab=151936)
ACT = dict(chunk=8, action_dim=7, proprio_dim=8, num_tokens=64,
           head_blocks=24, head_heads=8, head_dim=112, head_rope_base=10000.0, ln_eps=1e-5)
STOP_TOKEN_ID = 2
VDIM, PROJ_MID = DINO["hidden"] + SIG["hidden"], 4 * (DINO["hidden"] + SIG["hidden"])
...
def _strip(d: dict, prefix: str) -> dict:
    return {k[len(prefix):]: v for k, v in d.items() if k.startswith(prefix)}

def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--ckpt", type=Path, default=Path("/home/khanhnd61/data/VLA-Adapter/LIBERO-Object-Pro"))
    ...
    for k in ("chunk", "action_dim", "proprio_dim", "num_tokens", "head_blocks", "head_heads", "head_dim"):
    ...
    _add(writer, "action_queries.weight", M["action_queries.weight"])
    ...
    for i in range(LM["layers"]):
        ...
        _add(writer, Q + "attn_q.weight", M[P + "self_attn.q_proj.weight"]); _add(writer, Q + "attn_q.bias", M[P + "self_attn.q_proj.bias"])
        _add(writer, Q + "attn_k.weight", M[P + "self_attn.k_proj.weight"]); _add(writer, Q + "attn_k.bias", M[P + "self_attn.k_proj.bias"])
        _add(writer, Q + "attn_v.weight", M[P + "self_attn.v_proj.weight"]); _add(writer, Q + "attn_v.bias", M[P + "self_attn.v_proj.bias"])
    ...
    for i in range(ACT["head_blocks"]):
        ...
        for nm in ("q_proj", "k_self", "v_self", "k_adapter", "v_adapter", "k_task", "v_task", "o_proj"):
            _add(writer, Q + nm + ".weight", AH[P + nm + ".weight"]); _add(writer, Q + nm + ".bias", AH[P + nm + ".bias"])
        _add(writer, Q + "ffn_ln.weight", AH[P + "ffn.0.weight"]); _add(writer, Q + "ffn_ln.bias", AH[P + "ffn.0.bias"])
        _add(writer, Q + "ffn_lin.weight", AH[P + "ffn.1.weight"]); _add(writer, Q + "ffn_lin.bias", AH[P + "ffn.1.bias"])
        _add(writer, Q + "gating", AH[P + "gating_factor"])
