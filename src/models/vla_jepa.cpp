...
struct gguf_reader {
    ...
    bool open(const std::string & path) {
        ...
        if (!gctx) { std::fprintf(stderr, "vla(vla_jepa): gguf_init_from_file failed for %s\n", path.c_str()); return false; }
        fp = std::fopen(path.c_str(), "rb");
        if (!fp) { std::fprintf(stderr, "vla(vla_jepa): fopen failed for %s\n", path.c_str()); return false; }
    ...
    bool read_raw(const char * name, void * buf) {
        ...
        if (id < 0) { std::fprintf(stderr, "vla(vla_jepa): missing tensor %s\n", name); return false; }
    ...
    std::vector<float> read_f32(const char * name) {
        ...
        if (!t) { std::fprintf(stderr, "vla(vla_jepa): missing tensor %s\n", name); return {}; }
        ...
        else { std::fprintf(stderr, "vla(vla_jepa): tensor %s unsupported type %d\n", name, (int) t->type); return {}; }
        return out;
    }
    std::vector<uint8_t> read_convert(const char * name, ggml_type target) {
        ...
        std::fprintf(stderr, "vla(vla_jepa): unsupported resident type %d for %s\n", (int) target, name); return {};
    }

    bool fetch_rows_f32(const char * name, const std::vector<int32_t> & row_ids, float * dst, int64_t cols) {
        const ggml_tensor * t = meta(name);
        if (!t || t->ne[0] != cols || t->ne[2] != 1 || t->ne[3] != 1) { std::fprintf(stderr, "vla(vla_jepa): %s shape unfit for row-fetch\n", name); return false; }
        ...
        for (size_t k = 0; k < row_ids.size(); ++k) {
            ...
            if (r < 0 || r >= rows) { std::fprintf(stderr, "vla(vla_jepa): row %d out of range for %s\n", r, name); return false; }
