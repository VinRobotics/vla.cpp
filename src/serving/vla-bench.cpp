...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        auto need = [&](const char * name) -> const char * {
            if (i + 1 >= argc) { std::fprintf(stderr, "vla-bench: %s needs a value\n", name); std::exit(1); }
            return argv[++i];
        };
        if      (a == "--ckpt")     ckpt     = need("--ckpt");
    ...
    if (!hf.empty()) {
        if (!ckpt.empty()) { std::fprintf(stderr, "vla-bench: pass --ckpt or -hf, not both\n"); return 1; }
    ...
    if (!m) { std::fprintf(stderr, "vla-bench: model_load failed\n"); return 1; }
    ...
    for (size_t i = 0; i < noise.size(); ++i) noise[i] = 0.001f * (float) ((i * 2654435761u) % 1000) - 0.5f;
    ...
    for (int i = 0; i < warmup; ++i) {
        if (vla::predict(m, in).empty()) { std::fprintf(stderr, "vla-bench: predict failed\n"); vla::model_free(m); return 1; }
    }
    ...
    for (int i = 0; i < reps; ++i) {
        ...
        if (out.empty()) { std::fprintf(stderr, "vla-bench: predict failed at rep %d\n", i); vla::model_free(m); return 1; }
