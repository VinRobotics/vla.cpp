    ...
    def __init__(self, config: Eagle3_VLConfig, vision_model=None, language_model=None):
        ...
        if vision_model is not None:
            ...
            if config.vision_config.model_type == "intern_vit_6b":
                ...
                _vimpl = getattr(config.vision_config, "_attn_implementation", None)
                if _vimpl in (None, ""):
                    config.vision_config._attn_implementation = (
                        "flash_attention_2" if is_flash_attn_2_available() else "sdpa"
                    )
                elif _vimpl == "flash_attention_2" and not is_flash_attn_2_available():
        ...
        if language_model is not None:
            ...
            if config.text_config.architectures[0] == "LlamaForCausalLM":
                ...
                _timpl = getattr(config.text_config, "_attn_implementation", None)
                if _timpl in (None, ""):
                    config.text_config._attn_implementation = (
                        "flash_attention_2" if is_flash_attn_2_available() else "sdpa"
                    )
                    _timpl = config.text_config._attn_implementation
                elif _timpl == "flash_attention_2" and not is_flash_attn_2_available():
                    config.text_config._attn_implementation = "sdpa"
                    _timpl = "sdpa"
                assert _timpl in ("flash_attention_2", "sdpa", "eager"), (
                    f"Qwen2 attention must be flash_attention_2, sdpa, or eager; got {_timpl}"
                )
