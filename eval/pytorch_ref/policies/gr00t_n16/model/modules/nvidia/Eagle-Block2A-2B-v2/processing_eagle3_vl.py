...
class Eagle3_VLProcessor(ProcessorMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        force_fix_mistral_regex = kwargs.pop("fix_mistral_regex", True)
        try:
            ...
            if tokenizer is not None:
                tokenizer.init_kwargs["fix_mistral_regex"] = bool(force_fix_mistral_regex)
            return processor

        original_patch = TokenizersBackend._patch_mistral_regex.__func__

        @classmethod
        def _patch_mistral_regex_compat(
            patch_cls,
            tokenizer,
            ...
            **patch_kwargs,
            ...
            if fix_mistral_regex is None:
                fix_mistral_regex = force_fix_mistral_regex
            try:
                ...
                if not fix_mistral_regex or not hasattr(tokenizer, "pre_tokenizer"):
                    raise
                import tokenizers
        ...
        TokenizersBackend._patch_mistral_regex = _patch_mistral_regex_compat
        try:
            processor = super().from_pretrained(pretrained_model_name_or_path, **kwargs)
        finally:
            TokenizersBackend._patch_mistral_regex = classmethod(original_patch)
    ...
    def from_args_and_dict(cls, args, processor_dict: dict[str, Any], **kwargs):
        ...
        _validate_result = cls.validate_init_kwargs(processor_config=processor_dict, valid_kwargs=cls.valid_kwargs)
        if isinstance(_validate_result, tuple):
            unused_kwargs, valid_kwargs_dict = _validate_result
            processor = cls(*args, **valid_kwargs_dict)
        else:
            unused_kwargs = _validate_result
