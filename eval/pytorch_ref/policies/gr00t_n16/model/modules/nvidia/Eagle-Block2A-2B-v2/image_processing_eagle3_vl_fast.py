...
class Eagle3_VLImageProcessorFast(BaseImageProcessorFast):
    ...
    def preprocess(self, images: ImageInput, videos: VideoInput=None, **kwargs: Unpack[Eagle3_VLFastImageProcessorKwargs]) -> BatchFeature:
        ...
        # transformers 5.x renamed _prepare_input_images -> _prepare_image_like_inputs
        _prepare = getattr(self, "_prepare_image_like_inputs", None) or getattr(self, "_prepare_input_images")
        ...
        # In transformers 5.x, _further_process_kwargs already does this conversion
        ...
        # Filter to only kwargs accepted by _preprocess to avoid breakage
        # from new kwargs added in newer transformers versions.
        import inspect
        _valid_params = set(inspect.signature(self._preprocess).parameters.keys())
        _filtered = {k: v for k, v in kwargs.items() if k in _valid_params}
        if images is not None:
            return self._preprocess(images, **_filtered)
        elif videos is not None:
            return self._preprocess(videos, **_filtered)
