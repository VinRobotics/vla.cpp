...
def dynamic_preprocess(
    image, min_num=1, max_num=1, image_size=448, use_thumbnail=False
) -> List[Image.Image]:
...
class InternVL3Embedder(nn.Module):
    ...
    def get_fused_image_text_embedding_from_tensor_images(
        ...
        import os as _os
        _p = _os.environ.get("VLA_PROFILE") == "1"
        if _p:
            import time as _t
            torch.cuda.synchronize(); _v0 = _t.perf_counter()
        vit_embeds = self.model.extract_feature(pixel_values)
        if _p:
            torch.cuda.synchronize(); _v1 = _t.perf_counter()
        ...
        if _p:
            torch.cuda.synchronize(); _v2 = _t.perf_counter()
            print(f"[evo1-prof]   vit={1000*(_v1-_v0):.1f}ms fuse={1000*(_v2-_v1):.1f}ms "
                  f"tiles={sum(num_tiles_list)}", flush=True)
