...
DEFAULT_SWEEP = REPO_ROOT / "outputs" / "simpler_widowx"
...
def parse_summary(path: Path) -> dict:
    ...
    task_label = m_t.group(1) if m_t else None
    return {
        "successes": successes,
...
def collect_model(model_dir: Path) -> dict | None:
    ...
    for hit in sorted(model_dir.glob("**/summary.txt")):
        task = hit.parent.name
        if task in per_task:
            print(f"warning: multiple summaries for {model_dir.name} task '{task}': "
                  f"keeping {per_task[task]['_path']}, ignoring {hit}", file=sys.stderr)
            continue
        rec = parse_summary(hit)
        rec["_path"] = str(hit)
        per_task[task] = rec
    if not per_task:
        return None
...
def render_markdown(
    ...
    lines: list[str] = []
    lines.append(f"# SIMPLER sweep report - `{sweep.name}`")
    lines.append("")
    lines.append(f"- Sweep root: `{sweep}`")
    lines.append("- Harness: SimplerEnv WidowX/Bridge (named tasks)")
    ...
    lines.append("Sampled by the inline `mem_sampler` function in "
                 "[`eval/run_simpler.sh`](../../eval/run_simpler.sh) while "
    ...
    lines.append("- **Peak sys RAM** / **sys Δ** - peak system-wide used RAM "
                 ...
                 "baseline. On a co-resident SIMPLER run the Δ also includes SAPIEN "
                 "rendering, so it is an upper bound on the server's own footprint.")
    lines.append("")
    if not mem_stats:
        lines.append("_No `<arch>.mem.json` files found - these runs predate the sampler. "
                     "Re-run `eval/run_simpler.sh` to capture peak memory._")
        lines.append("")
    else:
    ...
    for name, per_task in per_task_all.items():
        ...
        for task in sorted(per_task):
            ...
            lines.append(
                f"| {task} | {t['successes']}/{t['n_episodes']} | "
...
def main() -> int:
    ...
    print("Harness: SimplerEnv WidowX/Bridge (named tasks)")
    print()
    ...
    print()
    sr_header = (
        f"{'model':<12} {'n_act':>6} {'tasks':>6} {'success':>10} {'terminated':>12} "
    ...
    for name, r in rows:
        ...
        print(
            f"{name:<12} {n_act_str:>6} {r['n_tasks']:>6} "
    ...
    if server_stats:
        ...
        srv_header = (
            f"{'model':<12} {'samples':>8} {'total (ms)':>12} "
        ...
        for name, _ in rows:
            s = server_stats.get(name)
            if s is None:
                print(f"{name:<12} {'(no log)':>8}")
                continue
            print(
                f"{name:<12} {s['n_samples']:>8} "
                f"{s['total']:>12.2f} "
                f"{s['vision']:>10.2f} "
                f"{s['inf']:>10.2f} "
                f"{s['other']:>10.2f}"
    ...
    if not mem_stats:
        print("  No <arch>.mem.json files found - legacy runs predate the sampler. "
              "Re-run eval/run_simpler.sh to capture peak memory.")
    else:
        mem_header = (f"{'model':<12} {'VRAM (MiB)':>12} {'RAM (MiB)':>12} "
        ...
        for name, _ in rows:
            m = mem_stats.get(name)
            if m is None:
                print(f"{name:<12} {'n/a':>12} {'n/a':>12} {'n/a':>13} {'n/a':>13} {'n/a':>9}")
                continue
            vram = m.get("peak_vram_mib")
    ...
    if args.per_task:
        for name, per_task in per_task_all.items():
            ...
            print(f"  {'task':<32} {'success':>10} {'terminated':>12} {'SR':>8} {'inf (ms)':>10}")
            for task in sorted(per_task):
                t = per_task[task]
                sr = t["successes"] / t["n_episodes"] if t["n_episodes"] else 0.0
                print(
